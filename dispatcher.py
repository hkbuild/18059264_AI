...
class Dispatcher:
      ...
      def _allocateFare(self, origin, destination, time):
          taxis = {}
          biddingTime = 3
          if self._parent.simTime - time > biddingTime:
              for taxiIdx in range(len(self._taxis)):
                  if len(self._taxis) > taxiIdx:
                      taxis[taxiIdx] = [1, 0, taxiIdx]
              if len(taxis) > 0:
                  for tId in taxis:
                      currentNode = self._parent.getNode(self._taxis[tId].currentLocation[0],
                                                         self._taxis[tId].currentLocation[1])
                      if self._taxis[tId]._passenger == None:
                          taxis[tId][0] = 1
                          Score = self._parent.travelTime(currentNode, self._parent.getNode(destination[0],
                                                                                                       destination[1]))
                          Score += 1
                          taxis[tId][1] = 100 / (Score)
                      else:
                          taxis[tId][0] = 1
                          currentDistNode = self._parent.getNode(self._taxis[tId]._path[-1][0],
                                                                 self._taxis[tId]._path[-1][1])
                          Score = self._parent.travelTime(currentNode, currentDistNode)
                          Score += 1
                          taxis[tId][1] = 100 / (Score)
              bestRoute = taxis[(list(taxis.keys())[0])][1]
              bestID = 0
              for i in taxis:
                  if taxis[i][0] == 1:
                      if taxis[i][1] < bestRoute:
                          bestRoute = taxis[i][1]
                          bestID = taxis[i][2]
              taxis[bestID][0] = 0
              self._fareBoard[origin][destination][time].taxi = bestID
              self._parent.allocateFare(origin, self._taxis[bestID])