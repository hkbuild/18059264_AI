...
class Taxi:
    ...
    def drive(self, newPose):
        ...
        if self._nextLoc is not None:
            ...
            if newPose[0] == self._nextLoc and newPose[1] == self._nextDirection:
                ...
                if self._loc is None:
                    nextNodee = newPose[0].occupy(newPose[1], self)
                else:
                    nextNodee = self._loc.vacate(self._direction, newPose[1])
                if nextNodee[0] == newPose[0] and nextNodee[1] == newPose[1]:
    ...
    def _planPath(self, origin, destination, **args):
        ...
        while len(expanded) > 0:
            ...
            nextExp = expanded[bestRoute]
            if destination in nextExp:
                return nextExp[destination]
            nextNode = nextExp.popitem()
            while len(nextExp) > 0 and nextNode[0] in explored:
                nextNode = nextExp.popitem()
            if len(nextExp) == 0:
                del expanded[bestRoute]
            if nextNode[0] not in explored:
                explored.add(nextNode[0])
                expTargets = [node for node in self._map[nextNode[0]].items() if node[0] not in explored]
